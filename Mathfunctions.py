...
class ControllSystem:
    def __init__(self, kp, ki, kd, sollwert, print_flag=True, dt=0.1, ambient_pressure = 1000):
        self.kp = kp
        self.ki = ki
        self.kd = kd
        self.dt = dt
    ...
    def logarithmicPID(self, istwert):
        ...
        rel_error = abs(self.sollwert - istwert) / self.sollwert
    ...
    def reset(self):
        ...
        self.integral_flag = False
        if self.print_flag:
            print(f"Previous Error: {self.prev_error} | Integral: {self.integral}")
    

class Interpolation:

    def __init__(self, sollwert=1000, print_flag=True):
