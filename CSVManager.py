...
class CSVReader:
    def __init__(self, pfad, print_flag=True, sep=';', decimal=',', encoding='cp1252', on_bad_lines='skip'):
        self.path = pfad
        self.sep = sep
    ...
    def extractData(self):
        try:
            ...
            self.Stellgroesse = clean_to_nan_or_float(data['Stellgröße']) if 'Stellgröße' in data.columns else None
    ...
    def extractDataForLUT(self):
        try:
            ...
            self.Ventilspannung_Durchlass = clean_to_nan_or_float(data['V_Durchlass'])
            self.Ventilspannung_Einlass = clean_to_nan_or_float(data['V_Einlass'])
            self.StufenDauer = data['Dauer'].values if 'Dauer' in data.columns else None
            # print(f"Erfolgreich konvertiert! Datentyp von Druck jetzt: {self.Druck.dtype}")
            # print(f"Erster Druckwert: {self.Druck[0]}, Letzter Druckwert: {self.Druck[-1]}")
            ...
            for i in range(1, len(self.stab_ventilspannung_einlass)):
                druck_aktuell = self.stab_druck[i]
                druck_vorher = self.stab_druck[i-1]
                if druck_aktuell > druck_vorher:
                    self.stab_v_einlass_steigend.append(self.stab_ventilspannung_einlass[i])
                    self.stab_v_durchlass_steigend.append(self.stab_ventilspannung_durchlass[i])
                    self.stab_druck_einlass_steigend.append(self.stab_druck[i])
                elif druck_aktuell < druck_vorher:
                    self.stab_v_einlass_fallend.append(self.stab_ventilspannung_einlass[i])
                    self.stab_v_durchlass_fallend.append(self.stab_ventilspannung_durchlass[i])
                    self.stab_druck_einlass_fallend.append(self.stab_druck[i])
...
class CreateFile:
    ...
    def pathRequest(self):
        ...
        try:
            ...
            with open(test_filename, mode='w') as f:
                pass 
            __import__('os').remove(test_filename)
    ...
    def allocateCSVfile(self, csv_location, stringchain = ['Zeit_s', 'Druck_mBar', 'V_Durchlass', 'V_Einlass', 'Stellgröße', 'P-Anteil', 'I-Anteil', 'D-Anteil', 'response']):
            self.filename = f"messung_{time.strftime('%Y%m%d-%H%M%S')}.csv"
            self.path = csv_location
            if self.path != "." and not self.path.endswith('\\') and not self.path.endswith('/'):
                        self.path += '\\'
            if self.path == ".":
                self.full_path = self.filename
    ...
    def allocateCSVfileforGUI(self, csv_location, stringchain = ['Zeit_s', 'Druck_mBar', 'V_Durchlass', 'V_Einlass', 'Stellgröße', 'P-Anteil', 'I-Anteil', 'D-Anteil', 'Stabilität']):
        self.filename = f"Live_Plot_Messung_{time.strftime('%Y%m%d-%H%M%S')}.csv"
        self.path = csv_location
        if self.path != "." and not self.path.endswith('\\') and not self.path.endswith('/'):
                    self.path += '\\'
        if self.path == ".":
    ...
    def writeToCSV(self, zeit, druck, v_durchlass, v_einlass, stellgroesse, duration, p_anteil, i_anteil, d_anteil, stabilität):
        t_str = f"{zeit:.3f}".replace('.', ',')
        p_str = f"{druck:.5f}".replace('.', ',')
        vd_str = f"{v_durchlass:.2f}".replace('.', ',')
        ve_str = f"{v_einlass:.2f}".replace('.', ',')
        stellgroesse_str = f"{stellgroesse:.5f}".replace('.', ',')
        p_anteil_str = f"{p_anteil:.5f}".replace('.', ',')
        i_anteil_str = f"{i_anteil:.5f}".replace('.', ',')
        d_anteil_str = f"{d_anteil:.5f}".replace('.', ',')
        stabilität_str = f"{stabilität}".replace('.', ',')
        self.buffer.append([t_str, p_str, vd_str, ve_str, stellgroesse_str, p_anteil_str, i_anteil_str, d_anteil_str, duration, stabilität_str])
        try:
    ...
    def writeToCSVforLUT(self, zeit, druck, v_durchlass, v_einlass, duration):
        t_str = f"{zeit:.3f}".replace('.', ',')
        p_str = f"{druck:.5f}".replace('.', ',')
        vd_str = f"{v_durchlass:.2f}".replace('.', ',')
        ve_str = f"{v_einlass:.2f}".replace('.', ',')
        if duration is None:
            ...
            dur_str = f"{duration:.2f}".replace('.', ',')
        self.buffer.append([t_str, p_str, vd_str, ve_str, dur_str])
        try:
            with open(self.full_path, mode='a', newline='', encoding = "utf-8") as f:
                writer = csv.writer(f, delimiter=';')
                while self.buffer:
