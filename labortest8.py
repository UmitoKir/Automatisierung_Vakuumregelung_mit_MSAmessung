...
ki= 0.01 #letzter stand 0.001 funtioniert könnte aber noch strärker sein 
#0.0005 #0.0001  # 0.2 #standartmäßig

kd = 0.0 #vllt 0.1 oder 0.05 #5e-6
dt = 0.1

Dauer = 60*60 #Dauer in Sekunden, die der Druck im Zielbereich bleiben soll, damit das Programm stoppt. (zusätzlich zum relativen Fehler von 1% und der Ableitung des Drucks von 0.001 mBar/s)
Max_dauer = 60*60*10 #sekunden * Minuten * Stunden
counter_limit = 500
...
def regelung(sensor, valves, lutData, measuredData, sollwert, startzeit):
    ...
    while lokale_zeit < Endzeit and istwert > 0.001: #relativer Fehler kleiner 1%
        ...
        Stellgröße, rel_fehler = controlUnit.logarithmicPID(istwert)
        v_durch = 10
        ...
        #Stabilitätscheck
        schwankung = (istwert - sensor.prev_pressure)/sensor.prev_pressure
        ...
        #wichtig hierbei muss die iteration durch die gesammte While-Schleife minimal höher als 0.1s insgesammt sein.   
        #Dafür ist diese while schleife da um dieses timing der kommunikation mit dem druck sensor zu stabilisieren. 
        #wenn t<0.1s => irgendwann zu timing error => komischer Wert für Druck(meist ein viel zu hoher Wert) interpretiert => regelschleife explodiert
